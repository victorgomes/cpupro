...
const command = cli
    ...
    .option('-t, --turbofan <path>', 'Specify the path to turbo.cfg to include Turbofan graphs in the report')
    .action(function ({args, options}) {
        ...
        const inputStream = cpuprofileFile !== '-' ? fs.createReadStream(cpuprofileFile) : process.stdin;
            ...
            try {
                ...
                for (const file of files) {
                    if (/^turbo-.*\.json$/.test(file)) {
                        const content = fs.readFileSync(path.join(turboDir, file), 'utf8');
                        let parsed;
                        try {
                            parsed = JSON.parse(content);
                        } catch (e) {
                            console.warn(`Ignoring incomplete or invalid Turbofan trace: ${file}`);
        ...
        readFromStream(inputStream, async data => {
            ...
            if (!options.open || options.outputDir || options.filename) {
                ...
                await report.writeToFile(filepath);
