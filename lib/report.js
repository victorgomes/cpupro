...
function createBase64DataPrinter(maxChunkSize, binary, compress) {
  return createHtmlBase64DataPrinter(
      maxChunkSize, compress,
      // type
      `discovery/${binary ? 'binary-' : ''}${
          compress ? 'compressed-' : ''}data-chunk`,
      // onDataChunk
      `discoveryLoader.push(chunk, ${binary}, ${compress})`);
}

function createRawTextDataPrinter(maxChunkSize) {
  return createHtmlRawTextPrinter(maxChunkSize,
                                  // type
...
module.exports = function createReport(data, filename, turbofanData) {
  ...
  const writeToFile = (filepath) => {
    ...
    let template = fs.readFileSync(reportTemplateFilename, 'utf8');
    template =
        template.replace(/dataSource:\s*"url"\s*,\s*data:\s*[a-zA-Z0-9_.]+/g,
                         'dataSource:"push",data:true');
    fs.writeFileSync(writtenToFile, template);
    ...
    result.push(`\n<script>discoveryLoader.start(${JSON.stringify({
      ...
      name :
          filename || (mainModule && path.relative(process.cwd(), mainModule)),
    ...
    result.push('\n<script>discoveryLoader.finish(' + encodedSize +
                ')</script>');

    if (turbofanData) {
      result.push('\n<script>window.__turbofanData = ' +
                  JSON.stringify(turbofanData) + ';\n</script>');
    }
