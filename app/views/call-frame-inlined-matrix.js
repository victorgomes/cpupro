...
discovery.view.define('call-frame-inlined-matrix', function (el, props, data, context) {
    ...
    for (let i = 0; i < snapshots.length; i++) {
        ...
        const currentSnapshot =
            mergeSnapshots && prevSnapshot?.hash === snapshot.hash
                ? prevSnapshot
                : {
                      hash: snapshot.hash,
    ...
    for (let i = 0; i < rowsCount; i++) {
        for (const snapshot of mergedSnapshots) {
            shapshotsRowsEl.append(
                createElement('div', snapshot.presence[i] ? 'snapshot-cell present' : 'snapshot-cell missed')
            );
        }
    }

    shapshotsRowsEl.style.setProperty('--snapshot-count', mergedSnapshots.length);
    el.append(treeEl, shapshotsRowsEl);
