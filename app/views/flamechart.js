...
const defaultTooltipContent = [
    {
        ...
        data: 'node.value',
        content: [
            {when: 'marker("package")', content: ['package-badge']},
            {when: 'marker("module")', content: ['module-badge']},
            {
                when: 'marker("call-frame")',
                content: [
                    'module-badge:module',
            ...
            {content: [{view: 'block', content: 'text:name'}]}
        ]
    },
    {
        view: 'duration',
...
const defaultDetailsContent = [
    {
        ...
        content: {
            ...
            content: [
                ...
                {content: ['badge:{ text: name, href: marker("category").href }']}
            ]
        }
    },
    {
...
discovery.view.define(
    ...
    function (el, config, data, context) {
        ...
        const enableScrollingEl = utils.createElement(
            'div',
            {
        ...
        const renderDetails = force => {
            const nextDetailsNodeIndex =
                selectedNodeIndex !== -1 ? selectedNodeIndex : zoomedNodeIndex > 0 ? zoomedNodeIndex : -1;
    ...
    `);
        ...
        const unsubscribeTimings = timings.subscribe(
            utils.debounce(
                () => {
                    timings.recompute?.();
                    chart.resetValues();
        ...
        const removeOnScrollListener = discovery.addHostElEventListener(
            'scroll',
            ({target}) => {
        ...
        destroyEl.onConnect = () => {
            ...
            if (firstPageContentChild) {
                ...
                removeOnPointerDownListener = () =>
                    firstPageContentChild.removeEventListener('pointerdown', enableScrolling, true);
            }
